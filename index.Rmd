...
<span style="color:red">Note:</span>The Amherst RStudio server has been updated to R version 3.6.1, so the previous issues regarding different results when working with random numbers are not a concern anymore. Everything should run the same both on your local installation, and on the online server.
...
If you get a `command not found` error, that means you don't have R installed and you can proceed with the steps below. If R is already on your machine, it's best to make sure you have the latest version - if your version is lower than 3.6.1, it might be a good idea to do a fresh reinstall of R.
...
Click on the `.pkg` download of the latest R release (as of late February 2020, that is 3.6.3, nicknamed "Holding the Windsock" as a reference to [this Peanut comic](https://www.gocomics.com/peanuts/1974/02/25)).
...
If you've ever installed anything on your Mac, you should be pretty comfortable with this process. Click **Continue**.

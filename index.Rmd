...
For Mac users, there's also two other ways, should the RStudio server not be currently available. Unfortunately, if you're a Windows user, those ways will not work for you. The reason is that the college's server is UNIX-based, as are Macs, so integration between the two is very easy. Windows is not UNIX-based, so it's more difficult to access a remote UNIX server. If you have a Windows machine, the easiest way to get your server files on your local machine is by using the download method, whever the RStudio server is working. Otherwise, go to IT - they might be able to help you out in other ways.

## Download (MacOS and Windows)

The most straightforward way to get all your files from the server is simply by downloading them. Log into the RStudio server (if it's working at the moment), and then look at the *Files* pane in the bottom right corner. Check the box next to all the files and folders you want to download.
...
If you selected multiple files or a folder, they will be downloaded as a zipped file (extension `.zip`). You can name that whatever you want. Click *Download*.
...
Double clicking on the `.zip `file will unzip it and yield a normal folder. The files are now on your local machine, and you can now move them wherever you want in your system, as you please.
...
Another way that works really nicely for Macs is mapping drives. Basically, you'll have all the server files mapped to your machine at all times, as if the server was just another folder in your memory. To begin, open the Finder menu, and go to `Go > Connect to Server...`.

![](img/transfer6.png)

In the text box that appears, type the address `smb://unix-mac.amherst.edu`. Then click *Connect*.
...
Again, this method only works from a Mac. It might also work on Windows if you have a Linux client installed, like Cygwin. Through the terminal, you can issue a command to copy all of your server files onto your local machine. This is probably the simplest method, if you're comfortable with the terminal. Don't worry if you don't understand what the commands below do - you can simply copy and paste them in, without worrying about what they mean.

Open up a fresh terminal window. You must create a directory to hold all of our files from the server - you can't just dump them all somewhere since that'd be messy. First, let's ensure you're in the home directory, by using the `cd` command. Type in the following and press `return`:
...
You'll see the prompt will have probably changed. Now type the following into the terminal and press `return`. This command takes **every single file** you have on the server and puts it into the *ServerFiles* folder. **Make sure to replace `your-username` with your Amherst username, e.g. jsmith22!**

```{bash, eval=FALSE}
scp -r 'your-username@romulus.amherst.edu:*' .
```

You'll be asked for your Amherst account password. Enter that, and wait - you'll see a lot of output as the server dumps every file you have on it locally. This might take a while, depending on how much stuff you have on the server, so feel free to go make some tea or buy a drink. If it's taking waaaay too long, you can terminate the process by pressing `Ctrl + C`.

If you know you only want one specific folder, you can instead use the following command, which is much faster since it doesn't copy every single thing on the server:

```{bash, eval=FALSE}
scp -r 'your-username@romulus.amherst.edu:directory-name' .
```

All your server files will now be in the *ServerFiles* folder, which in turn is in the home directory.
...
Now go to RStudio. In the top menu, go to `File > New Project...`. 
...
Now paste the key you copied from GitHub into the field that says "Repository URL" (`Cmd + V` on Mac, `Ctrl + V` on Windows). The Project directory name should get filled in automatically with the name of your repo. Choose whatever directory you would like to clone the repo in, and click *Create Project*.
...
That's it! Now you should see your changes in the GitHub repo. Follow the same workflow for every Git project you have in RStudio.
